...
class infill(ds_project_base):
    ...
    def get_resource(self):
        ...
        self._ubinfill_dir   = resource['UBINFILL_DIR']
...
# UBDL LOCATION
UBDL_DIR=/cluster/tufts/wongjiradlab/kmason03/uboonecode/ubdl
...
PORT={}
BROKER={}
SCRIPT=/cluster/tufts/wongjiradlab/larbys/pubs/dlleepubs/ubdlserver/run_infill_client.sh

COMMAND="source run_infill_client.sh $SUPERA_INPUTPATH $INFILL_OUTFILE $WORKDIR"

mkdir -p $WORKDIR
mkdir -p $INFILL_OUTPATH_DIR
module load singularity
singularity exec $CONTAINER bash -c "cd $WORKDIR && cp $SCRIPT . && $COMMAND && cp $INFILL_OUTFILE $INFILL_OUTPATH_DIR"
"""
            print self._treename
            print infillout
            submit = submitscript.format(jobtag,workdir,run,subrun,
                                         self._container,self._ubinfill_dir,workdir,
                                         supera,os.path.dirname(infillout),os.path.basename(infillout),
