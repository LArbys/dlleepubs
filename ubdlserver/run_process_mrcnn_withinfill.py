...
class mrcnn(ds_project_base):

    # Define project name as class attribute
    _project = 'mrcnn_withinfill'
    ...
    def get_resource(self):
        ...
        self._ubmrcnn_dir    = resource['UBMRCNN_DIR']
    ...
    def process_newruns(self):
        ...
        if nremaining<=0:
            self.info("Already running (%d) max number of mrcnn jobs (%d)" % (len(results),self._max_jobs) )
            return False
        ...
        for x in results:
            ...
            ubmrcnnout = dbdir + "/" + self._outfile_format%("ubmrcnn-withinfill-larcv-%s"%(self._runtable),run,subrun)
            ...
            submitscript="""#!/bin/sh
#
#SBATCH --job-name=ubmrcnnclient_{}
#SBATCH --output={}/log_ubmrcnnclient_{}_{}.txt
#SBATCH --ntasks=1
#SBATCH --time=8:00:00
...
# LOCATION OF MRCNNSERVER CODE (IN CONTAINER)
UBMRCNN_BASEDIR={}
...
PORT={}
BROKER={}
LOGFILE={}/log_mrcnn_withinfill_{}_{}_{}.txt

ENVSETUP="cd $UBDL_DIR && source setenv.sh && source configure.sh && cd ublarcvserver && source configure.sh"
COMMAND="start_ublarcvserver_ubmrcnn_client.py -l $LOGFILE -d tcp://$BROKER:$PORT -i $SUPERA_INPUTPATH -o $UBMRCNN_OUTFILE -a infill"

mkdir -p $WORKDIR
mkdir -p $UBMRCNN_OUTPATH_DIR
module load singularity
singularity exec $CONTAINER bash -c "$ENVSETUP && cd $UBMRCNN_BASEDIR && . $UBMRCNN_BASEDIR/setenv.sh && cd $WORKDIR && export && $COMMAND && cp $UBMRCNN_OUTFILE $UBMRCNN_OUTPATH_DIR"
"""
            print self._treename
            print ubmrcnnout
            submit = submitscript.format(jobtag,workdir,run,subrun,
                                         self._container,self._ubmrcnn_dir,workdir,
                                         supera,os.path.dirname(ubmrcnnout),os.path.basename(ubmrcnnout),
                                         self._port,self._broker,
                                         workdir,self._runtable,run,subrun)
    ...
    def validate(self):
        ...
        # get job listing
        psinfo = os.popen( "squeue | grep mrcnnc" )
        ...
        self._api._cursor.execute(query)
        results = self._api._cursor.fetchall()
        self.info("Number of mrcnn jobs in running state: %d"%(len(results)))
        for x in results:
            run    = int(x[0])
            subrun = int(x[1])
            dbdata = x[2]
        ...
        self._api._cursor.execute(query)
        results = self._api._cursor.fetchall()
        self.info("Number of mrcnn jobs in finished state: %d"%(len(results)))
        ...
        for x in results:
            ...
            pcheck = os.popen("%s/./singularity_check_mrcnn_jobs.sh %s %s %s"%(PUB_UBDLSERVER_DIR,ubmrcnnout,supera,PUB_UBDLSERVER_DIR))
    ...
    def modstatus(self):
        ...
        for x in results:
            ...
            if rs not in rslist:
                ...
                continue
            dbdir = self._out_dir + "/%03d/%02d/%03d/%02d/"%(rundiv100,runmod100,subrundiv100,subrunmod100)
            mrcnnout = dbdir + "/" + self._outfile_format%("mrcnnserveroutv2-larcv",run,subrun)
            cmd = "rm -f "+mrcnnout
