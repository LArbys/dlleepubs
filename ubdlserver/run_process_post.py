...
class post(ds_project_base):
    ...
    def get_resource(self):
        ...
        self._ubpost_dir     = resource['UBPOST_DIR']
    ...
    def process_newruns(self):
        ...
        if nremaining<=0:
            self.info("Already running (%d) max number of post jobs (%d)" % (len(results),self._max_jobs) )
            return False
        ...
        query =  "select t1.run,t1.subrun,supera,reco2d,opreco"
        ...
        for x in results:
            ...
            reco2d = x[3]
            opreco = x[4]
            larflow = supera.replace("stage1","ubdlserver").replace("supera", "larflow-noinfill-larcv-%s"%(self._runtable))
            ssnet   = supera.replace("supera","ssnetserveroutv2-larcv")
            ubmrcnn = supera.replace("stage1","ubdlserver").replace("supera", "ubmrcnn-noinfill-larcv-%s"%(self._runtable))
            
            ubpost_llout = dbdir + "/" + self._outfile_format%("ubpost-noinfill-larlite-%s"%(self._runtable),run,subrun)
            ubpost_lcout = dbdir + "/" + self._outfile_format%("ubpost-noinfill-larcv-%s"%(self._runtable),run,subrun)

            ubclust_llout = dbdir + "/" + self._outfile_format%("ubclust-noinfill-larlite-%s"%(self._runtable),run,subrun)
            ...
            submitscript="""#!/bin/sh
#
#SBATCH --job-name=ubpostclient_{}
#SBATCH --output={}/log_ubpostclient_{}_{}.txt
#SBATCH --ntasks=1
#SBATCH --time=8:00:00
...
UBPOST_OUTDIR={}
UBPOST_LARLITE_OUTFILE={}
UBPOST_LARCV_OUTFILE={}
UBCLUST_OUTFILE={}
...
COMMAND_POST="dev -c $LARFLOW_INPUTPATH -su $SUPERA_INPUTPATH -re $RECO2D_INPUTPATH -op $OPRECO_INPUTPATH -wss $SSNET_INPUTPATH -oll $UBPOST_LARLITE_OUTFILE -olc $UBPOST_LARCV_OUTFILE"
COPY_POST_OUT="cp $UBPOST_LARLITE_OUTFILE $UBPOST_OUTDIR && cp $UBPOST_LARCV_OUTFILE $UBPOST_OUTDIR"

#COMMAND_CLUST="dev_mrcnn_cluster $UBPOST_OUTDIR/$UBPOST_LARLITE_OUTFILE $SUPERA_INPUTPATH $MRCNN_INPUTPATH $UBCLUST_OUTFILE"
COMMAND_CLUST="dev_mrcnn_cluster $UBPOST_LARLITE_OUTFILE $SUPERA_INPUTPATH $MRCNN_INPUTPATH $UBCLUST_OUTFILE"
COPY_CLUST_OUT="cp $UBCLUST_OUTFILE $UBPOST_OUTDIR"
...
# clear old
rm -f $UBPOST_OUTDIR/$UBPOST_LARLITE_OUTFILE
rm -f $UBPOST_OUTDIR/$UBPOST_LARCV_OUTFILE
rm -f $UBPOST_OUTDIR/$UBCLUST_OUTFILE

echo "$COMMAND_POST"
srun singularity exec $CONTAINER bash -c "$ENVSETUP && cd $WORKDIR && $COPY_UBSPLIT_CFG && $COMMAND_POST && $COPY_POST_OUT"

echo "$COMMAND_CLUST"
srun singularity exec $CONTAINER bash -c "$ENVSETUP && cd $WORKDIR && $COMMAND_CLUST && $COPY_CLUST_OUT"
"""
            submit = submitscript.format(jobtag,workdir,run,subrun,
                                         self._container,self._ubpost_dir,workdir,
                                         supera,larflow,opreco,reco2d,ssnet,ubmrcnn,
                                         os.path.dirname(ubpost_llout),
                                         os.path.basename(ubpost_llout),
                                         os.path.basename(ubpost_lcout),
                                         os.path.basename(ubclust_llout))
    ...
    def validate(self):
        ...
        # get job listing
        psinfo = os.popen( "squeue | grep ubpostcl" )
        ...
        self._api._cursor.execute(query)
        results = self._api._cursor.fetchall()
        self.info("Number of post jobs in running state: %d"%(len(results)))
        for x in results:
            run    = int(x[0])
            subrun = int(x[1])
            dbdata = x[2]
        ...
        self._api._cursor.execute(query)
        results = self._api._cursor.fetchall()
        self.info("Number of post jobs in finished state: %d"%(len(results)))
        ...
        for x in results:
            ...
            pcheck = os.popen("%s/./singularity_check_post_jobs.sh %s %s %s"%(PUB_UBDLSERVER_DIR,ubpost_lcout,supera,PUB_UBDLSERVER_DIR))
    ...
    def error_handle(self):
        ...
        for x in results:
            ...
            ubpost_lcout = dbdir + "/" + self._outfile_format%("ubpost-noinfill-larcv-%s"%(self._runtable),run,subrun)
            ubclust_llout = dbdir + "/" + self._outfile_format%("ubclust-noinfill-larlite-%s"%(self._runtable),run,subrun)
            os.system("rm -f %s"%(ubpost_llout))
            os.system("rm -f %s"%(ubpost_lcout))
            os.system("rm -f %s"%(ubclust_llout))
    ...
    def modstatus(self):
        ...
        for x in results:
            ...
            if rs not in rslist:
                ...
                continue
            dbdir = self._out_dir + "/%03d/%02d/%03d/%02d/"%(rundiv100,runmod100,subrundiv100,subrunmod100)
            postout = dbdir + "/" + self._outfile_format%("postserveroutv2-larcv",run,subrun)
            cmd = "rm -f "+postout
