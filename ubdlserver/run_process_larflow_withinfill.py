...
class larflow(ds_project_base):
    ...
    def process_newruns(self):
        ...
        query =  "select t1.run,t1.subrun,infill"
        ...
        for x in results:
            ...
            larflowout = dbdir + "/" + self._outfile_format%("larflow-withinfill-larcv-%s"%(self._runtable),run,subrun)
...
"""
    ...
    def validate(self):
        ...
        for x in results:
            ...
            subrundiv100 = subrun/100
            dbdir        = self._out_dir + "/%03d/%02d/%03d/%02d/"%(rundiv100,runmod100,subrundiv100,subrunmod100)
            larflowout   = dbdir + "/" + self._outfile_format%("larflow-infill-larcv-%s"%(self._runtable),run,subrun)
            jobtag       = 10000*run + subrun
            workdir      = self._grid_workdir + "/%s_%04d_%03d"%(self._project,run,subrun)            
