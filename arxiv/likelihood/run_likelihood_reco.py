...
class ll_reco(ds_project_base):
    ...
    def process_newruns(self):
        ...
        for x in results:
            ...
            inputlist_f.write("%s" % trackerana_input.replace("90-days-archive",""))
            inputlist_f.close()

            # shower ana
            inputlist_f = open(os.path.join(inputlistdir,"shower_ana_inputlist_%05d.txt"% int(jobtag)),"w+")
            inputlist_f.write("%s" % showerana_input.replace("90-days-archive",""))
            inputlist_f.close()

            # pkl
            inputlist_f = open(os.path.join(inputlistdir,"rst_pkl_inputlist_%05d.txt"% int(jobtag)),"w+")
            inputlist_f.write("%s" % rst_pkl_input.replace("90-days-archive",""))
            inputlist_f.close()

            # all vertex
            inputlist_f = open(os.path.join(inputlistdir,"all_ana_inputlist_%05d.txt"% int(jobtag)),"w+")
            inputlist_f.write("%s" % vertexana_input.replace("90-days-archive",""))
            inputlist_f.write(" %s" % multipid_input.replace("90-days-archive",""))
            inputlist_f.write(" %s" % showerqualsingle_input.replace("90-days-archive",""))
            inputlist_f.write(" %s" % shower_truth_match_input.replace("90-days-archive",""))
            inputlist_f.write(" %s" % tracker_ana_out_input.replace("90-days-archive",""))
            inputlist_f.write(" %s" % track_pgraph_match_input.replace("90-days-archive",""))
            inputlist_f.write(" %s" % trackqualsingle_input.replace("90-days-archive",""))
            inputlist_f.write(" %s" % track_truth_match_input.replace("90-days-archive",""))
            inputlist_f.write(" %s" % FinalVertexVariables_input.replace("90-days-archive",""))
            inputlist_f.close()
            ...
            if len(self._cosmic_inter)>0:
                run_data = run_data.replace("BBB",cosmic_inter.replace("90-days-archive",""))

            else:
                run_data = run_data.replace("BBB","\"\"")

            if len(self._flash_inter)>0:
                run_data = run_data.replace("CCC",flash_inter.replace("90-days-archive",""))
            else:
