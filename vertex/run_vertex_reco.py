...
class vertex_reco(ds_project_base):
    ...
    def get_input_output( self, run, subrun, jobtag ):
        ...
        _,ssnetinput,inputdbdir,outdbdir = cast_run_subrun(run,subrun,
                                                           self._runtag,self._file_format,
                                                           self._input_dir,self._out_dir)

        vertex_recoout    = os.path.join(outdbdir,self._file_format%("vertex_reco-out",run,subrun))
        vertex_recoana    = os.path.join(outdbdir,self._file_format%("vertex_reco-ana",run,subrun))
        vertex_recopickle = os.path.join(outdbdir,self._file_format%("vertex_reco-pickle",run,subrun))

        vertex_recoout    += ".root"
        vertex_recoana    += ".root"
        vertex_recopickle += ".pkl"
        
        return ssnetinput, vertex_recoout, vertex_recoana, vertex_recopickle, inputdbdir, outdbdir
    ...
    def process_newruns(self):
        ...
        for x in results:
            ...
            # job variables
            jobtag,ssnetinput,inputdbdir,outdbdir = cast_run_subrun(run,subrun,
                                                                    self._runtag,self._file_format,
                                                                    self._input_dir,self._out_dir)
    ...
    def validate(self):
        ...
        for x in results:
            ...
            ssnetinput, vertex_recoout, vertex_recoana, vertex_recopickle, inputdbdir, outputdbdir = self.get_input_output( run, subrun, jobtag )
            workdir = os.path.join(self._grid_workdir,"%s_%04d_%03d"%(self._project,run,subrun))

            # link
            vertex_recoout = os.path.join(outputdbdir,"vertexout_%d.root" % jobtag)
            vertex_recoana = os.path.join(outputdbdir,"vertexout_%d.root" % jobtag)
            vertex_pkl1    = os.path.join(outputdbdir,"ana_comb_df_%d.pkl" % jobtag)
            vertex_pkl2    = os.path.join(outputdbdir,"ana_truth_df_%d.pkl" % jobtag)
            vertex_pkl3    = os.path.join(outputdbdir,"ana_vertex_df_%d.pkl" % jobtag)
