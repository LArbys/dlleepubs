...
class vertex_reco(ds_project_base):
    ...
    def get_resource(self):
        ...
        self._sub_script       = os.path.join(SCRIPT_DIR,str(resource['SUBMIT_SCRIPT']))
        self._py_script        = str(resource['PYRUN_SCRIPT'])
    ...
    def process_newruns(self):
        ...
        for x in results:
            ...
            larcvtruthinput  = os.path.join(inputdbdir,self._file_format%("larcvtruth",run,subrun))
            larcvtruthinput += ".root"
            ...
            # copy reco job template over
            self.info("copy python script")
            cmd("scp -r %s %s" % (self._py_script,workdir))
            run_script = os.path.join(workdir,os.path.basename(self._run_script))
            ...
            sub_data=sub_data.replace("JJJ",str(jobtag))

            # input files
            sub_data=sub_data.replace("WWW",superainput.replace("90-days-archive",""))
            sub_data=sub_data.replace("XXX",ssnetinput.replace("90-days-archive",""))
            sub_data=sub_data.replace("YYY",taggerinput.replace("90-days-archive",""))
            sub_data=sub_data.replace("ZZZ",larcvtruthinput.replace("90-days-archive",""))
            ...
            if True: # use this bool to turn off for testing
                ...
                if interactive == True:
                    ...
                    SS = "sbatch %s" % os.path.join(workdir,os.path.basename(self._sub_script))
