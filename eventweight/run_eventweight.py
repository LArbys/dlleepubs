...
class eventweight(ds_project_base):

    # Define project name as class attribute
    _project = 'eventweight'
    ...
    def __init__(self,project_name):
        ...
        super(eventweight,self).__init__()
    ...
    def get_resource(self):
        ...
        self._sub_script       = os.path.join(SCRIPT_DIR,"submit_pubs_job_long.sh")
    ...
    def query_queue(self):
        """ data about slurm queue pertaining to eventweight jobs"""
        psqueue = commands.getoutput("squeue | grep eventwei")
    ...
    def process_newruns(self):
        ...
        if nremaining<=0:
            self.info("Already running (%d) max number of eventweight jobs (%d)" % (len(results),self._max_jobs) )
            return
        ...
        query += " where t1.status=1 and t3.status=3"
        ...
        for x in results:
            ...
            # prepare work dir
            workdir      = os.path.join(self._grid_workdir,"ew",self._out_runtag,"%s_%04d_%03d"%(self._project,run,subrun))
            ...
            mcinfo_input = os.path.join(inputdbdir0,self._file_format%("mcinfo",run,subrun))
            mcinfo_input+= ".root"
            ...
            # copy fcls
            stat,out = commands.getstatusoutput("scp -r %s %s" % (self._reader_fcl1,workdir))
            reader_fcl1 = os.path.basename(self._reader_fcl1)
            ...
            run_data = run_data.replace("KKK",reader_fcl1)
            run_data = run_data.replace("RRR",reader_fcl2)
            ...
            sub_data=sub_data.replace("AAA","eventweight_%s" % str(jobtag))
            ...
            if True: # use this bool to turn off for testing
                ...
                if interactive == True:
                    ...
                    SS = "sbatch %s" % os.path.join(workdir,"submit_pubs_job_long.sh")
            ...
            print "submissionok=",submissionok
            if submissionok:
    ...
    def validate(self):
        ...
        # get job listing
        psinfo = os.popen( "squeue | grep eventwei")
        ...
        self._api._cursor.execute(query)
        results = self._api._cursor.fetchall()
        self.info("Number of eventweight jobs in running state: %d"%(len(results)))
        for x in results:
            run    = int(x[0])
            subrun = int(x[1])
        ...
        self._api._cursor.execute(query)
        results = self._api._cursor.fetchall()
        self.info("Number of eventweight jobs in finished state: %d"%(len(results)))
        for x in results:
            run    = int(x[0])
            subrun = int(x[1])
            ...
            # link
            output_file = os.path.join(outdbdir,"evweight_art_%d.root" % jobtag)
...
if __name__ == '__main__':
    ...
    test_obj = eventweight(sys.argv[1])
    jobslaunched = False
